#!/usr/bin/env python
"""
Fire Brigade Zone Climate Projections - PURE RELATIVE RISK (SIMPLIFIED)
...
- Native 50m resolution matching training data (NO downsampling)
"""
...
TIME_STEPS = 60
SPATIAL_WINDOW_SIZE = 4  # Must match training (4×4 window = 200m context at 50m resolution)
STATIC_VARS = [
...
with rioxarray.open_rasterio(template_path) as template_raster:
    template = template_raster.rio.clip(aoi.geometry, all_touched=True, drop=True).squeeze('band')
