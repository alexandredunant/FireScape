"""
Temporal Generalisability Analysis for FireScape Dataset (Parquet Version)

This version reads from the spacetime_dataset.parquet file instead of the netCDF
stack, making it more lightweight and faster to run.

Analyzes the distribution of fire observations across:
- Years
- Months
- Seasons
- Day of year
...
PARQUET_PATH = os.path.join(OUTPUT_DIR, "spacetime_dataset.parquet")
...
try:
    ...
    try:
        ...
        print("\nPlease update PARQUET_PATH in the script to point to your spacetime_dataset.parquet file")
        exit(1)
...
event_dates = pd.to_datetime(df['date'])
labels = df['bin'].values if 'bin' in df.columns else df['label'].values
...
# Create temporal features
df['year'] = event_dates.dt.year
df['month'] = event_dates.dt.month
df['day_of_year'] = event_dates.dt.dayofyear
df['month_name'] = event_dates.dt.strftime('%B')
df['month_abbr'] = event_dates.dt.strftime('%b')
...
# Add label name
label_col = 'bin' if 'bin' in df.columns else 'label'
df['label_name'] = df[label_col].map({0: 'No Fire', 1: 'Fire'})
df['label'] = df[label_col]  # Standardize column name
...
sns.heatmap(heatmap_data, annot=True, fmt='d', cmap='YlOrRd',
            ax=ax9, cbar_kws={'label': 'Count'}, linewidths=0.5)
...
date_range_years = pd.to_datetime(df['date']).max().year - pd.to_datetime(df['date']).min().year + 1
total_possible_months = date_range_years * 12
...
# Additional insights
print(f"\n--- Key Insights ---")
print(f"Most fire-prone month: {fire_by_month.idxmax()} ({fire_by_month.max()} events)")
print(f"Least fire-prone month: {fire_by_month.idxmin()} ({fire_by_month.min()} events)")
print(f"Most fire-prone season: {fire_by_season.idxmax()} ({fire_by_season.max()} events)")
print(f"Year with most fires: {fire_by_year.idxmax()} ({fire_by_year.max()} events)")
print(f"Year with least fires: {fire_by_year.idxmin()} ({fire_by_year.min()} events)")
...
print(f"  - Statistics: {OUTPUT_STATS_PATH}")
